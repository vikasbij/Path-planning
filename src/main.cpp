...
int main() {
  ...
  double reference_vel = 0.0; //have a reference velocity to target
  

  h.onMessage([&reference_vel,&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy,&lane](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			for (int i=0; i < sensor_fusion.size(); i++)
			{
				...
				{
					if((check_car_s > car_s) && ((check_car_s-car_s)<target_s))
					{
					   front=true; 
					}
				}
				else if((current_lane == lane-1) && (check_car_s-car_s)<target_s && (check_car_s>car_s-(target_s)*0.2))
				{
					left=true;
				}
				else if (current_lane==lane+1 && (check_car_s-car_s)<target_s && (check_car_s>car_s-(target_s)*0.2))
			...
			if(front==true)
			{
				if(left==false and lane>0)
				{
					lane =lane-1;
					reference_vel-=0.5*0.224;
				}
				else if (right==false and lane!=2)
				{
				lane=lane+1;
				reference_vel-=0.5*0.224;
				}
			     else reference_vel-=0.224;
			}
			else 
			{
				if(reference_vel<49.5)
				{
					reference_vel+=0.224;
				}
			}
			...
			else 
			{
				...
				double ref_x_previous = previous_path_x[previous_size-2];
				double ref_y_previous = previous_path_y[previous_size-2];
